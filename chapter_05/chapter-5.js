...
const livingLang = SCRIPTS.filter((el) => el.living);
const ttbScripts = SCRIPTS.filter((el) => el.direction === "ttb");
const nameOfrtlScripts = SCRIPTS.filter((el) => el.direction === "rtl").map(
  (el) => el.name
);

// console.log(livingLang.length, ttbScripts.length);
// console.log(nameOfrtlScripts);

function characterCount(script) {
  const reducer = (acc, [from, to]) => {
    return acc + (to - from);
  };

  return script.ranges.reduce(reducer, 0);
}

const biggestScript = SCRIPTS.reduce((acc, el) => {
  return characterCount(acc) < characterCount(el) ? el : acc;
});

// console.log(biggestScript);
// console.log(characterCount(biggestScript));
